...
# Loading the datasets from Kaggle folder
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, "kaggle_dataset")
...
model_path = os.path.join(BASE_DIR, "Symptom_Checker.pkl")
...
def home():
    ...
    if not symptoms or not isinstance(symptoms, list):
        return jsonify({"error": "Invalid or missing symptoms input"}), 400
    ...
    # Convert precautions row (numpy array) to list
    my_precautions = precautions[0].tolist() if precautions else []

    # Crash-safe parsing & convert to list
    try:
        medications = ast.literal_eval(medications[0]) if medications and medications[0] else []
        if isinstance(medications, (np.ndarray, pd.Series)):
            medications = medications.tolist()
    except Exception as e:
        print("❌ Failed to parse medications:", medications[0] if medications else None, e)
        medications = []

    try:
        rec_diet = ast.literal_eval(rec_diet[0]) if rec_diet and rec_diet[0] else []
        if isinstance(rec_diet, (np.ndarray, pd.Series)):
            rec_diet = rec_diet.tolist()
    except Exception as e:
        print("❌ Failed to parse diet:", rec_diet[0] if rec_diet else None, e)
        rec_diet = []

    workout_list = workout.tolist() if workout is not None else []
